...
parser.add_argument('--train-index-count', type=int, default=1000000)
...
if not os.path.exists(args.faiss_index + ".trained"):
    ...
    random_sample = np.random.choice(np.arange(vals.shape[0]), size=[min(args.train_index_count, vals.shape[0])], replace=False)
    start = time.time()
