...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def load_state_dict(self, state_dict, strict=True, args=None):
        """we rewrite the load state dict here for only load part of model
        add by  
        """
        if self.decoder.knn_lambda_type == 'trainable' \
            ...
            print("not load model's {}".format(remove_keys))
            for k in remove_keys:
                # print("not load model's {}".format(k))
    ...
    def add_args(parser):
        ...
        parser.add_argument("--only-label-count-as-feature", default=False, action="store_true")
        ...
        # we add 4 arguments for adapter
    ...
    def forward(
            ...
            need_self_attention_weights: bool = False,
        ...
        decoder_out = self.decoder(
            ...
            need_self_attn=need_self_attention_weights,
            activate_adapter=activate_adapter
        )
        return decoder_out
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        if activate_adapter and self.embedding_append_adapter:
        ...
        for layer in self.layers:
            x = layer(x, encoder_padding_mask, activate_adapter=activate_adapter)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.only_label_count_as_feature = args.only_label_count_as_feature
        ...
        if self.knn_lambda_type == "trainable" and self.knn_k_type == "trainable":
            ...
            self.retrieve_result_to_k_and_lambda = nn.Sequential(
                nn.Linear(args.max_k if not self.label_count_as_feature or self.only_label_count_as_feature
                          else args.max_k * 2,
            ...
            if not self.only_label_count_as_feature:
                nn.init.xavier_normal_(self.retrieve_result_to_k_and_lambda[0].weight[:, : args.k], gain=0.01)
            else:
                nn.init.xavier_normal_(self.retrieve_result_to_k_and_lambda[0].weight[:, : args.k], gain=0.01)

            # nn.init.uniform_(self.retrieve_result_to_k_and_lambda[0].weight[:, : args.k], a=0.01)
    ...
    def forward(
        ...
        x, extra = self.extract_features(
            ...
            alignment_layer=alignment_layer,
            alignment_heads=alignment_heads,
            need_self_attn_weights=need_self_attn,
            activate_adapter=activate_adapter
        )
        ...
        if self.use_knn_datastore:
            ...
            if self.only_label_count_as_feature:
                label_counts = self.knn_datastore.get_label_count_segment(tgt_index, relative=self.relative_label_count)
                network_inputs = label_counts.detach().float()

            elif self.label_count_as_feature:
    ...
    def extract_features(
            ...
            alignment_layer: Optional[int] = None,
            alignment_heads: Optional[int] = None,
            need_self_attn_weights: bool = False,
            activate_adapter: bool = False,
    ):
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, unused_ret, self_attn_weights = layer(
                x,
                ...
                need_self_attn_weights=need_self_attn_weights and bool((idx == alignment_layer)),
                activate_adapter=activate_adapter
            )
            inner_states.append(x)
            if layer_attn is not None and idx == alignment_layer:
                attn = layer_attn.float().to(x)
            if self_attn_weights is not None and idx == alignment_layer:
                self_attn = self_attn_weights.float().to(x)  # [bsz, tgt len, prev tgt len]

        # print('self_attn', self_attn.size())
        ...
        # the default is average heads
        # self_attn = attn.mean(dim=0)  # [num_heads, bsz, tgt_len, src_len]
        ...
        return x, {"attn": [attn], "inner_states": inner_states, 'self_attn': self_attn, "last_hidden": x}
