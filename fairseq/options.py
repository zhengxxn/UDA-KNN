...
def get_interactive_generation_parser(default_task="translation"):
    parser = get_generation_parser(interactive=True, default_task=default_task)
    add_knn_record_args(parser)
    return parser
...
def get_save_datastore_parser(default_task=None):
    # add by, just copy from get_validation_parser
...
def get_reference_knn_search_parser(default_task=None):
    parser = get_parser("Validation", default_task)
    add_dataset_args(parser, train=True)
    add_distributed_training_args(parser, default_world_size=1)
    add_datastore_args(parser)
...
def add_datastore_args(parser):
    ...
    group.add_argument("--save-attn-weights", default=False, action='store_true')

    # for denoising task
    group.add_argument("--save-denoising-feature", default=False, action='store_true')
    group.add_argument("--activate-adapter", default=False, action="store_true")

    # for save empty source
    group.add_argument("--save-source-empty-feature", default=False, action="store_true")

    # for save enc ordered output
    group.add_argument("--enc-output-save-dir", default=None, type=str, help="save dir for enc hidden state")
    group.add_argument("--enc-dstore-size", metavar="N", default=1, type=int,
                       help="datastore size")
    group.add_argument("--enc-state-dim", metavar="N", default=1, type=int,
                       help="enc hidden state")
...
def add_denoising_args(parser):
    group = parser.add_argument_group("Denoising")

    group.add_argument('--remove-denoising-inputs', default=False, action='store_true')
...
def add_knn_record_args(parser):
    ...
    group.add_argument('--record-uncertainty', default=False, action='store_true')
    group.add_argument('--record-knn-candidate', default=False, action='store_true')
    group.add_argument('--record-knn-candidate-count', type=int, default=1)
    group.add_argument('--record-attention', default=False, action='store_true')
    group.add_argument('--record-self-attention', default=False, action='store_true')
    group.add_argument('--knn-record-file', type=str, default=None)
    group.add_argument('--record-candidate-count', type=int, default=1)
